(function() {
  ...
  angular.module('omega').config(function($stateProvider, $urlRouterProvider, $httpProvider, $animateProvider) {
...
(function() {
  angular.module('omega').controller('IoCtrl', function($scope, $rootScope) {
    ...
    $scope.restoreLocal = function(content) {
      return $rootScope.resetOptions(content).then((function() {
        $scope.importSuccess();
        return $rootScope.updateProfile()["finally"](function() {
    ...
    return $scope.restoreOnline = function() {
      return $.ajax({
...
(function() {
  angular.module('omega').controller('PacProfileCtrl', function($scope) {
    ...
    return $scope.$watch('profile', onProfileChange, true);
  });
...
(function() {
  angular.module('omega').controller('SwitchProfileCtrl', function($scope, $location, $modal, profileIcons, getAttachedName, omegaTarget, $timeout) {
    var advancedConditionTypesExpanded, basicConditionTypeSet, basicConditionTypesExpanded, expandGroups, exportRuleList, oldLastUpdate, oldRuleList, oldSourceUrl, onAttachedChange, stopWatchingForGuide, type, unwatchRules, updateHasConditionTypes, _i, _len;
    ...
    $scope.$watch('options["-showConditionTypes"]', function(show) {
      ...
      if ($scope.showConditionTypes === 0) {
        $scope.conditionTypes = basicConditionTypesExpanded;
        return $scope.setExportRuleListHandler(exportRuleList);
      } else {
    ...
    $scope.resetRules = function() {
      ...
      return $modal.open({
        ...
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          rule = _ref[_j];
          _results.push(rule.profileName = $scope.defaultProfileName);
        }
        return _results;
      });
    };
    ...
    $scope.$watch('profile.defaultProfileName', function(name) {
      ...
      if (!$scope.attached || !$scope.attachedOptions.enabled) {
        return $scope.defaultProfileName = name;
      }
    });
      ...
      if (enabled) {
        ...
        if ($scope.profile.defaultProfileName === $scope.attachedName) {
          if ($scope.attached) {
            ...
            return $scope.defaultProfileName = $scope.attached.defaultProfileName;
          } else {
            $scope.profile.defaultProfileName = 'direct';
            return $scope.defaultProfileName = 'direct';
          }
        }
      }
    });
    ...
    $scope.$watch('defaultProfileName', function(name) {
      if ($scope.attached && $scope.attachedOptions.enabled) {
    ...
    return stopWatchingForGuide = $scope.$watch('profile.rules', function(rules) {
      if (!rules) {
        return;
      }
      stopWatchingForGuide();
      return omegaTarget.state(['web.switchGuide', 'firstRun']).then(function(_arg) {
        var firstRun, switchGuide;
        switchGuide = _arg[0], firstRun = _arg[1];
